...
import { Upload, Play, BookOpen, GraduationCap, FileText, Layers, Calendar, Users, Info, X, Pen, Keyboard } from 'lucide-react';
...
export const SetupScreen: React.FC = () => {
    ...
    return (
        <div className="h-full w-full bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-6 sm:p-4 overflow-hidden">
            <div className="bg-white w-full max-w-6xl h-full rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">

                {/* Branding Side - Final Layout (Solid Teal & Glass) */}
                <div className="bg-primary-blue text-white p-8 md:w-1/3 h-full flex flex-col items-center justify-between text-center relative overflow-hidden z-10 shadow-xl">
                    {/* Subtle Texture for Depth */}
                    <div className="absolute inset-0 bg-white/5 opacity-10 pattern-grid-lg"></div>

                    {/* Main Content Area */}
                    <div className="flex-1 flex flex-col items-center justify-start w-full z-10 px-4 pt-24">
                        {/* Logo Card - Glass Effect */}
                        <div className="mb-6 p-6 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm shadow-md transform transition-transform duration-300 hover:scale-110">
                        ...
                        <div className="space-y-3 text-center">
                            ...
                            <p className="text-white/80 text-sm font-light opacity-90 pt-1">
                ...
                <div className="p-6 md:w-2/3 flex flex-col overflow-y-auto">
                    <div className="mb-4 border-b pb-3">
                        ...
                        <p className="text-gray-500 text-base">اختر بيانات الحصة وملف الكتاب قبل بدء الدرس.</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="block text-base font-bold text-slate-700">المرحلة الدراسية</label>
                                <select
                            ...
                            <div className="space-y-2">
                                <label className="block text-base font-bold text-slate-700">النظام</label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="block text-base font-bold text-slate-700">الصف</label>
                                <select
                            ...
                            <div className="space-y-2">
                                <label className="block text-base font-bold text-slate-700">الفصل</label>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="block text-base font-bold text-slate-700">الفصل الدراسي</label>
                                <div className="relative">
                            ...
                            <div className="space-y-2">
                                <label className="block text-base font-bold text-slate-700">المادة</label>
                                <div className="relative">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                <label className="block text-base font-bold text-slate-700">الوحدة الدراسية <span className="text-slate-400 text-sm font-normal">(اختياري)</span></label>
                                <div className="flex bg-gray-100 p-0.5 rounded-lg">
                                    <button
                                        type="button"
                                        onClick={() => handleChange('unitMode', 'text')}
                                        className={`px-3 py-1 rounded-md text-sm font-bold transition-all flex items-center gap-1.5 ${lessonSetup.unitMode === 'text' ? 'bg-white shadow text-gray-800' : 'text-gray-500'}`}
                                    >
                                        <Keyboard size={14} />
                                        كتابة
                                    </button>
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-md text-sm font-bold transition-all flex items-center gap-1.5 ${lessonSetup.unitMode === 'drawing' ? 'bg-white shadow text-gray-800' : 'text-gray-500'}`}
                                    >
                                        <Pen size={14} />
                                        قلم
                                    </button>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                <label className="block text-base font-bold text-slate-700">موضوع الدرس</label>
                                <div className="flex bg-gray-100 p-0.5 rounded-lg">
                                    <button
                                        type="button"
                                        onClick={() => handleChange('lessonTitleMode', 'text')}
                                        className={`px-3 py-1 rounded-md text-sm font-bold transition-all flex items-center gap-1.5 ${lessonSetup.lessonTitleMode === 'text' ? 'bg-white shadow text-gray-800' : 'text-gray-500'}`}
                                    >
                                        <Keyboard size={14} />
                                        كتابة
                                    </button>
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-md text-sm font-bold transition-all flex items-center gap-1.5 ${lessonSetup.lessonTitleMode === 'drawing' ? 'bg-white shadow text-gray-800' : 'text-gray-500'}`}
                                    >
                                        <Pen size={14} />
                                        قلم
                                    </button>
                        ...
                        {/* Row 6: PDF Upload */}
                        <div className="space-y-2">
                            <label className="block text-base font-bold text-slate-700">ملف الدرس (PDF)</label>
                            <div
                                onClick={() => fileInputRef.current?.click()}
                                className={`border-2 border-dashed rounded-xl p-2 flex items-center justify-center gap-1.5 cursor-pointer hover:border-primary-blue hover:bg-blue-50 transition-all group ${errors.pdfFile ? 'border-red-600 bg-red-50' : 'border-gray-300'}`}
                            >
                                <div className="p-2 bg-gray-100 rounded-full group-hover:bg-white transition-colors">
                                    <Upload className="text-gray-500 group-hover:text-primary-blue" size={20} />
                                </div>
                        ...
                        <div className="space-y-2">
                            <button
                                type="submit"
                                className="w-full bg-primary-green text-white font-bold py-3 rounded-xl shadow-lg hover:bg-opacity-90 hover:shadow-xl transition-all flex items-center justify-center gap-2 text-lg mt-4 transform hover:-translate-y-1"
                            >
